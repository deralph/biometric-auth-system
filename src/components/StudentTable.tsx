...
const StudentsTable: React.FC<{ students: Student[] }> = ({ students }) => {
  ...
  const filtered =
    filter === "All Students"
      ? students
      : students.filter((s) => s.level === filter);
      ...
      <div className="flex flex-wrap items-center gap-3  pb-4 mb-4">
        {levels.map((l) => (
          <button
            key={l}
            onClick={() => setFilter(l)}
            className={`text-xs px-3 py-1 font-bold rounded ${
              filter === l
      ...
      <div className="overflow-hidden rounded-xl ">
        ...
        <div className="">
          {filtered.map((s, idx) => (
            <div
              key={s.id}
              className="grid grid-cols-12 gap-4 p-4 border-2 border-gray-300 mt-4 rounded-md items-center"
            >
        ...
        <div className="p-4 flex justify-end">
          <button className="bg-blue-500 text-white px-8 py-2 rounded-md font-boold">
            View all
          </button>
        </div>
      </div>
